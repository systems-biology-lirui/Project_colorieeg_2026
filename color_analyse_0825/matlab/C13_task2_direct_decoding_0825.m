%% ========================================================================
% 脚本名称: C13_task2_direct_decoding_0825.m
% 功能:
%   1. 【Task 2 记忆色彩直接二分类 Decoding (不考虑水果种类)】
%      将灰色水果试次直接划分为单纯的两组：记忆红色 (草莓+西瓜) vs 记忆绿色 (猕猴桃+卷心菜)
%   2. 【分层 5 折交叉验证 (Stratified 5-Fold CV)】
%      严格保证训练集/测试集红绿平衡，无跨水果限制
%   3. 【多频段联合与 6 单频段独立解码】
...
fig_dir    = fullfile(res_root, 'figures', 'decoding_task2_direct');
tc_tab_dir = fullfile(res_root, 'tables', 'decoding_task2_direct_timecourses');
sum_csv    = fullfile(res_root, 'tables', 'task2_direct_decoding_summary.csv');
sum_mat    = fullfile(res_root, 'tables', 'task2_direct_decoding_summary.mat');
...
fprintf('========================================================================\n');
fprintf('  【C13: Task 2 记忆色彩直接二分类 Decoding (不考虑水果种类)】\n');
...
for s_i = 1:numel(unique_subs)
    ...
    % 仅提取灰色水果试次
    gray_mask = strcmp(ti.state, 'gray');
    ti_gray   = ti(gray_mask, :);
    n_gray    = height(ti_gray);
    
    % 二分类标签: 全部记忆红 = 1 (草莓+西瓜), 全部记忆绿 = 0 (猕猴桃+卷心菜)
    y = zeros(n_gray, 1);
    y(strcmp(ti_gray.memory_color, 'red')) = 1;
    
    % 构建分层 5 折交叉验证掩码 (直接两组数据，不管水果)
    rng(42);
    idx_red   = find(y == 1);
    idx_green = find(y == 0);
    ...
    fold_tr_masks = false(n_gray, cfg.n_folds);
    fold_te_masks = false(n_gray, cfg.n_folds);
    ...
    fprintf('>>> [被试 %d/%d: %s] 载入完成 (%.2fs), 灰色试次: %d (红: %d, 绿: %d), 包含 %d 个显著通道\n', ...
        s_i, numel(unique_subs), sub_id, toc(t_load), n_gray, sum(y==1), sum(y==0), n_sub_elecs);
    
    for e_i = 1:n_sub_elecs
        ch_name = sub_elecs{e_i};
        global_idx = global_idx + 1;
        
        fig_png = fullfile(fig_dir, sprintf('%s_%s_task2_direct_decoding.png', sub_id, ch_name));
        tc_csv  = fullfile(tc_tab_dir, sprintf('%s_%s_task2_direct_timecourse.csv', sub_id, ch_name));
        ...
        % 提取滑动时间窗特征 [n_gray x n_bands x n_win]
        X_3d = zeros(n_gray, cfg.n_bands, n_win, 'single');
        for b = 1:cfg.n_bands
            b_name = cfg.bands{b};
            raw_b = squeeze(ep.(b_name)(gray_mask, ch_idx, :));
            for w = 1:n_win
                tc = t_centers(w);
        ...
        if need_calc
            ...
            sgtitle(sprintf('%s - %s (Task 2 Direct Memory Decoding)', sub_id, ch_name), ...
