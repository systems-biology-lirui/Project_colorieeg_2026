%% test_run_sub008_rsa.m
% 验证 sub008 的所有 22 个同向显著通道的 RSA 双图生成及汇总统计
clear; clc; close all;
...
% 4. 预载入该被试全部 6 频段数据至 RAM
fprintf('[+] 正在载入 %s 的 6 频段数据至内存 ...\n', sub_id);
t_load = tic;
...
% 6. 逐通道循环计算并出图
summary_records = cell(n_elecs, 8);
...
for e_i = 1:n_elecs
    ...
    if isempty(e_idx3) || isempty(e_idx2)
        warning('通道 %s 在 Task2 或 Task3 中缺失，跳过。', elec);
        continue;
    end
    ...
    % 时程距离与效应
    d_phys_time = squeeze(rdms_4_time(3, 4, :))'; 
    d_memo_time = squeeze(rdms_4_time(1, 2, :))'; 
    d_same_time = (squeeze(rdms_4_time(1, 3, :))' + squeeze(rdms_4_time(2, 4, :))') / 2; 
    d_diff_time = (squeeze(rdms_4_time(1, 4, :))' + squeeze(rdms_4_time(2, 3, :))') / 2; 
    ...
    net_avg    = d_diff_avg - d_same_avg;
    
    post_mask = (t_centers >= 100) & (t_centers <= 600);
    [max_eff, max_idx] = max(effect_s(post_mask));
    post_centers = t_centers(post_mask);
    peak_lat = post_centers(max_idx);
    
    summary_records{e_i, 1} = sub_id;
    summary_records{e_i, 2} = elec;
    summary_records{e_i, 3} = d_phys_avg;
    summary_records{e_i, 4} = d_memo_avg;
    summary_records{e_i, 5} = d_same_avg;
    summary_records{e_i, 6} = d_diff_avg;
    summary_records{e_i, 7} = net_avg;
    summary_records{e_i, 8} = max_eff;
    
    % --- 绘制图 1: 平均图 (左 10 RDM, 中 4 RDM, 右 3D MDS) ---
    h1 = figure('Units', 'pixels', 'Position', [50, 100, 1600, 480], 'Color', 'w', 'Visible', 'off');
    ...
    if size(Y_3d, 2) < 3
        ...
        var_exp = 100 * eigvals(1:3) / max(1e-6, sum(abs(eigvals(eigvals > 0))));
    end
    ...
    savefig(h1, fig1_path);
    exportgraphics(h1, png1_path, 'Resolution', 300);
    close(h1);
    
    % --- 绘制图 2: 随时间的变化图 (左 10 条件, 中 4 类别, 右 净效应) ---
    h2 = figure('Units', 'pixels', 'Position', [50, 100, 1600, 480], 'Color', 'w', 'Visible', 'off');
    
    subplot(1, 3, 1);
    hold on;
    xline(0, 'k:', 'LineWidth', 1.0, 'HandleVisibility', 'off');
    d_sw = smoothdata(squeeze(rdms_10_time(1, 2, :))', 'gaussian', smooth_pts);
    d_kc = smoothdata(squeeze(rdms_10_time(3, 4, :))', 'gaussian', smooth_pts);
    d_sk = smoothdata(squeeze(rdms_10_time(1, 3, :))', 'gaussian', smooth_pts);
    d_wc = smoothdata(squeeze(rdms_10_time(2, 4, :))', 'gaussian', smooth_pts);
    
    plot(t_centers, d_sw, 'Color', [0.85, 0.20, 0.20], 'LineWidth', 1.6, 'DisplayName', 'Strawberry vs Watermelon (Red Memo)');
    plot(t_centers, d_kc, 'Color', [0.20, 0.70, 0.25], 'LineWidth', 1.6, 'DisplayName', 'Kiwi vs Cabbage (Green Memo)');
    plot(t_centers, d_sk, 'Color', [0.45, 0.45, 0.85], 'LineWidth', 1.6, 'DisplayName', 'Strawberry vs Kiwi (Diff Memo)');
    plot(t_centers, d_wc, 'Color', [0.70, 0.40, 0.80], 'LineWidth', 1.6, 'DisplayName', 'Watermelon vs Cabbage (Diff Memo)');
    hold off;
    ...
    savefig(h2, fig2_path);
    exportgraphics(h2, png2_path, 'Resolution', 300);
    close(h2);
    
    fprintf('  [%d/%d] 通道 %s-%s 完成 (耗时 %.2f 秒) | Net Effect Avg = %.4f | Peak = %.4f (@%d ms)\n', ...
        e_i, n_elecs, sub_id, elec, toc(t_ch), net_avg, max_eff, round(peak_lat));
end

% 导出 sub008 汇总表
tbl_sub008 = cell2table(summary_records, 'VariableNames', ...
    {'subject', 'channel', 'd_phys_avg', 'd_memo_avg', 'd_same_avg', 'd_diff_avg', 'net_avg', 'peak_effect'});
out_csv = fullfile(root_dir, 'result', 'tables', 'rsa_sub008_channel_summary.csv');
writetable(tbl_sub008, out_csv);
fprintf('\n[+] sub008 全部 %d 个通道处理完成！汇总表已保存至:\n    %s\n', n_elecs, out_csv);
