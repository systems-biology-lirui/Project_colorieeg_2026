...
# Load color_with_sti electrodes directly from ieegloc.xlsx AAL3 column
...
if os.path.exists(loc_path):
    ...
    if aal_col:
        ...
        target_elecs = df_loc[is_color_sti]['Channel'].astype(str).unique().tolist()
print(f"[{subject}] Using {len(target_elecs)} color_with_sti electrodes from excel: {target_elecs}")
...
def plot_for_feature(feature_type, mat_path):
    ...
    feature_path = mat_path
    if not os.path.exists(feature_path) and feature_type == 'subband_60_150' and subject == 'test001':
        feature_path = os.path.join(pipeline_dir, 'feature', 'subband_60_150', 'task1_hg_subband.mat')
        
    if not os.path.exists(feature_path):
        print(f"File not found: {feature_path}")
        return

    out_dir = os.path.join(pipeline_dir, 'images', subject, 'channel_colorsti', feature_type)
    os.makedirs(out_dir, exist_ok=True)
    
    if feature_type == 'subband_60_150':
        mat = read_mat(feature_path)
        epoch = mat['epoch']
