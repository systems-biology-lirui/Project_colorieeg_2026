...
# -*- coding: utf-8 -*-
"""
Task 1: Plot test006 A1-A5 Color vs Gray time courses and bar-scatters.
Task 2: Plot 9 electrodes gray fruit comparison (ANOVA) in task2.
"""
...
# 任务 1：test006 A1-A5 差异电极列表
T1_SUBJECTS = {
    'test006': ['A1', 'A2', 'A3', 'A4', 'A5']
}
...
# ==================== 绘图函数 2: 4-Way Gray Fruit Comparison ====================
...
def run_task1():
    print("="*60)
    print("RUNNING TASK 1: test006 A1-A5 Color vs Gray Plotting")
    print("="*60)
    for subj, elecs in T1_SUBJECTS.items():
        for elec in elecs:
            print(f"\nProcessing T1: {subj}, Electrode: {elec}...")
            # ERP
            try:
                ...
                print(f"  Saved ERP to: {out_path}")
                shutil.copy(out_path, os.path.join(artifacts_dir, out_name))
            except Exception as e:
                print(f"  [Error] T1 ERP failed for {subj}-{elec}: {e}")
            ...
            try:
                ...
                print(f"  Saved HG to: {out_path}")
                shutil.copy(out_path, os.path.join(artifacts_dir, out_name))
            except Exception as e:
                print(f"  [Error] T1 HG failed for {subj}-{elec}: {e}")

def run_task2():
    print("\n" + "="*60)
    print("RUNNING TASK 2: 9 Electrodes Gray Fruits Comparison in Task2")
    print("="*60)
    for subj, elec in T2_ELECTRODES:
        print(f"\nProcessing T2: {subj}, Electrode: {elec}...")
        # ERP
        try:
            data, ch_list, time_ms = load_task2_data(subj, is_hg=False)
            ch_idx = ch_list.index(elec)
            out_name = f"{subj}_erp_{elec}_gray_fruits.png"
            out_path = os.path.join(kaojin_dir, out_name)
            plot_gray_fruits_diff(subj, elec, ch_idx, data, time_ms, out_path, is_hg=False)
            print(f"  Saved ERP Gray Fruits Comparison to: {out_path}")
            shutil.copy(out_path, os.path.join(artifacts_dir, out_name))
        except Exception as e:
            print(f"  [Error] T2 ERP failed for {subj}-{elec}: {e}")
            
        # HG
        try:
            data, ch_list, time_ms = load_task2_data(subj, is_hg=True)
            ch_idx = ch_list.index(elec)
            out_name = f"{subj}_hg_{elec}_gray_fruits.png"
            out_path = os.path.join(kaojin_dir, out_name)
            plot_gray_fruits_diff(subj, elec, ch_idx, data, time_ms, out_path, is_hg=True)
            print(f"  Saved HG Gray Fruits Comparison to: {out_path}")
            shutil.copy(out_path, os.path.join(artifacts_dir, out_name))
        except Exception as e:
            print(f"  [Error] T2 HG failed for {subj}-{elec}: {e}")

def load_task2_data(subj, is_hg):
    return load_data(subj, is_hg, task_num=2)
