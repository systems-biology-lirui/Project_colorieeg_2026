...
result_dir = os.path.join(analyse_dir, 'result', 'electrode_D_features')
set_path = os.path.join(base_dir, 'seegdata', 'test1', 'erp1.set')

# 确保输出目录存在
os.makedirs(result_dir, exist_ok=True)

# 目标电极列表（需要D7来做D6的拉普拉斯重参考）
target_chans = ['D1', 'D2', 'D3', 'D4', 'D5', 'D6', 'D7']
plot_chans = ['D1', 'D2', 'D3', 'D4', 'D5', 'D6']
...
# 重参考后的数据：前 6 个通道对应 D1-D6
ref_data = np.zeros((len(plot_chans), raw_data.shape[1]))

# D1：双极重参考 D1 - D2
ref_data[0, :] = data_ordered[0, :] - data_ordered[1, :]

# D2-D6：一维 Laplacian 重参考 D_i - 0.5 * (D_{i-1} + D_{i+1})
for i in range(1, 6): # 对应 D2(1) 到 D6(5)
...
feature_configs = {
    ...
    '30-70Hz_Gamma': {
        ...
        'h_freq': 70.0,
        'notch': [50],
        'baseline': (-0.2, 0.0), # 用于计算 Z-score 的基线段
...
for name, cfg in feature_configs.items():
    ...
    if cfg['type'] == 'erp':
        ...
        epochs_data = epochs.get_data() * 1e6 # 转换为微伏以保持一致
        times = epochs.times
    ...
    # 5. 反向获取各条件的 index，对每个电极画图
    event_codes = epochs.events[:, 2] # 各个 epoch 的 event 编码
    # 构建反向映射 {trig_name: event_code}
