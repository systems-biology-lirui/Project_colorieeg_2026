...
def resample_10ms_bins(data, time_ms, start_time=-100.0, end_time=700.0):
    ...
    # 每个 10ms 包纳 5 个原始时间点 (500Hz 采样率下 5 * 2ms = 10ms)
    bin_size = 5
    n_bins = len(t_indices) // bin_size
    
    # 重新整理数据形状: (Rep, n_ch, n_bins)
...
def fit_eval_tg_row_strategy1(t_tr, X_tr_r, X_tr_g, test_pairs_resampled):
    """
    对于给定的 t_train，用色块数据训练，并在所有 t_test 上预测 4 种配对
    """
    # 训练特征与标签
    ...
    row_accs = np.zeros((4, n_bins)) # (4 pairs, n_t_test)
...
def fit_eval_tg_row_strategy2(t_tr, train_pairs_resampled, X_te_r, X_te_g):
    """
    对于给定的 t_train，用 4 种灰色水果配对训练，并在色块数据的所有 t_test 上测试
    """
...
def main():
    print("="*85)
    print("Step 3_2: Cross-Decoding Temporal Generalization ERP Analysis")
    print("="*85)
    
    # 1. 读取 ERP memory_color 电极列表
    sig_path = os.path.join(doc_dir, 'select_channel_memory_significance_erp.csv')
    if not os.path.exists(sig_path):
        print("[ERROR] select_channel_memory_significance_erp.csv 丢失，跳过计算")
        return
    ...
    if not active_subjs:
        print("[WARNING] 无任何被试包含 memory_color 显著 ERP 电极！")
        return
    ...
    for subj in active_subjs:
        print(f"\n>>> Computing temporal generalization for {subj}...")
        ...
        # 垂直堆叠为 2D 矩阵 (TrainingTime * TestingTime)
        ...
        plot_tg_heatmap(
            ...
            f"TG Strategy 1 (Color-to-Gray-Memory): {subj}",
            os.path.join(out_fig_dir, f"strategy1_{subj}_temporal_generalization.png")
        )
        plot_tg_heatmap(
            matrix_strat2, time_bins, 
            f"TG Strategy 2 (Gray-Memory-to-Color): {subj}",
            os.path.join(out_fig_dir, f"strategy2_{subj}_temporal_generalization.png")
        )
        
    # 3. 计算 Group 平均时间泛化热图并绘制
    ...
    plot_tg_heatmap(
        ...
        "Group TG Strategy 1 (Color-to-Gray-Memory)",
        os.path.join(out_fig_dir, "strategy1_group_temporal_generalization.png")
    )
    plot_tg_heatmap(
        group_strat2, time_bins,
        "Group TG Strategy 2 (Gray-Memory-to-Color)",
        os.path.join(out_fig_dir, "strategy2_group_temporal_generalization.png")
    )
    
    # 4. 导出矩阵明细为 CSV (为了防止保存 2D 矩阵为常规 1D CSV 过大，我们平铺为三列: TrainTime, TestTime, Acc)
    export_rows_strat1 = []
    export_rows_strat2 = []
    ...
    xlsx_s1 = os.path.join(doc_dir, 'cross_decoding_tg_strategy1.xlsx')
    csv_s1 = os.path.join(doc_dir, 'cross_decoding_tg_strategy1.csv')
    xlsx_s2 = os.path.join(doc_dir, 'cross_decoding_tg_strategy2.xlsx')
    csv_s2 = os.path.join(doc_dir, 'cross_decoding_tg_strategy2.csv')
    ...
    print(f"\n[SUCCESS] Exported cross decoding temporal generalization tables to:\n  - {xlsx_s1}\n  - {xlsx_s2}")
    print("="*85)
