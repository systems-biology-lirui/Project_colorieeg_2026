...
data_path    = 'D:\Desktop\seeg\seegdata\test2';   % 数据文件夹路径
...
% 保存 ERP 数据
pop_saveset(EEG_ERP, 'filename', 'processed_ERP.set', 'filepath', data_path);
...
% 7. 基线矫正 (-250 到 -50ms)
EEG_TFA = pop_rmbase(EEG_TFA, [-250 -50]);

% 8. 去除坏段 (使用与 ERP 相同的阈值保持 trial 对齐)
% EEG_TFA = pop_eegthresh(EEG_TFA, 1, 1:EEG_TFA.nbchan, -300, 300, -0.5, 1.0, 0, 1);

% 保存 TFA 数据
pop_saveset(EEG_TFA, 'filename', 'processed_TFA.set', 'filepath', data_path);
...
mat_filename = fullfile(data_path, sprintf('task%d_ERP_epoched.mat',task_label));
...
mat_filename = fullfile(data_path, sprintf('task%d_TFA_epoched.mat',task_label));
save(mat_filename, 'epoch');
fprintf('成功将干净的 Epoch 矩阵及标签保存至: %s\n', mat_filename);
end
