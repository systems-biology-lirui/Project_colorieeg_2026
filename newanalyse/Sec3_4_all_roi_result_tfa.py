...
TASKS = [
    ...
    # },
    {
        ...
        'title': 'Task 1 Color vs Gray Per-Category Decoding (TFA)',
        'description': 'Decode color vs gray within each of 4 categories (face/body/object/scene) using multi-band TFA features, then average AUC across categories.',
...
def main():
    ...
    batch_root = str(get_within_decoding_batch_dir(BASE_PATH, 'tfa', SUBJECT, batch_name=BATCH_NAME))
    ...
    task_outputs = {}
    for task in TASKS:
        output_dir = str(get_within_decoding_task_dir(BASE_PATH, task['id'], 'tfa', SUBJECT, get_perm_tag(), variant='with_sti', batch_name=BATCH_NAME))
...
def build_task1_pair_holdout_data(mat):
    if 'tfa_task1' not in mat:
        raise ValueError('Missing matrix: tfa_task1')
    data = mat['tfa_task1']
    if data.shape[0] < 8:
...
def build_task2_gray_memory_combos(mat):
    if 'tfa_task2' not in mat:
        raise ValueError('Missing matrix: tfa_task2')
    data = mat['tfa_task2']
    gray_green = [2, 5]
    gray_red = [8, 11]
