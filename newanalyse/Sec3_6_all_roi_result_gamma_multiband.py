...
TASKS = [
    ...
    # },
    {
        'id': 'task1_color_vs_gray_per_category_multiband',
        'title': 'Task 1 Color vs Gray Per-Category Decoding (Gamma Multiband)',
        'description': 'Decode color vs gray within each category using 30-100 Hz multiband gamma features, then average AUC across categories.',
        'mode': 'within_category_color_gray',
        'folder': 'task1_color_vs_gray_per_category_multiband'
    },
]
...
def main():
    ...
    batch_root = str(get_within_decoding_batch_dir(BASE_PATH, 'gamma_multiband', SUBJECT, batch_name=BATCH_NAME))
    os.makedirs(batch_root, exist_ok=True)
    log_path = os.path.join(batch_root, f'batch_log_gamma_multiband_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')
    logger = make_logger(log_path)

    logger(f'Start multiband gamma decoding for {SUBJECT}')
    logger(f'Feature dir: {FEATURE_DIR}')
    logger(f'ROI count: {len(mat_files)}')

    task_outputs = {}
    for task in TASKS:
        output_dir = str(get_within_decoding_task_dir(BASE_PATH, task['id'], 'gamma_multiband', SUBJECT, get_perm_tag(), variant='with_sti', batch_name=BATCH_NAME))
...
def run_task_for_roi(fpath, roi_name, task, save_path, roi_plot_dir, logger):
    ...
    if mode == 'within_cv':
        ...
        if 'gmb_band_names' in mat:
            payload['band_names'] = mat['gmb_band_names']
        if 'gmb_band_ranges' in mat:
            payload['band_ranges'] = mat['gmb_band_ranges']
...
def build_task1_pair_holdout_data(mat):
    if 'gmb_task1' not in mat:
        raise ValueError('Missing matrix: gmb_task1')
    data = mat['gmb_task1']
    if data.shape[0] < 8:
...
def build_task2_gray_memory_combos(mat):
    if 'gmb_task2' not in mat:
        raise ValueError('Missing matrix: gmb_task2')
    data = mat['gmb_task2']
    gray_green = [2, 5]
    gray_red = [8, 11]
...
def run_decoding_over_time_group_holdout(X, y, groups, shuffle=False, seed=None):
    ...
    for ti, t in enumerate(time_indices):
        X_t = X[:, :, t]
        for gi, group in enumerate(uniq_groups):
            test_idx = groups == group
            train_idx = ~test_idx
...
def plot_latency_bar_scatter(latency_dict, title_text, y_label, save_path):
    ...
    if not clean_items:
        return
    clean_items.sort(key=lambda item: item[1])
    rois = [item[0] for item in clean_items]
    latencies = [item[1] for item in clean_items]
...
def baseline_zscore(X, baseline_end_idx):
    baseline = X[:, :, :baseline_end_idx]
    mu = baseline.mean(axis=2, keepdims=True)
    sd = baseline.std(axis=2, keepdims=True) + 1e-8
    return (X - mu) / sd
...
def run_decoding_per_category(mat, category_pairs, shuffle=False, seed=None):
    ...
    for ci, (X_cat, y_cat) in enumerate(category_data):
        scores = run_decoding_over_time_cv(X_cat, y_cat, n_repeats=N_REPEATS_REAL, shuffle=False, seed=RANDOM_STATE)
    ...
    def one_perm(perm_seed):
        ...
        for X_cat, y_cat in category_data:
            scores = run_decoding_over_time_cv(X_cat, y_cat, n_repeats=N_REPEATS_PERM, shuffle=True, seed=perm_seed)
            perm_aucs.append(np.mean(scores, axis=1))
        perm_aucs = np.stack(perm_aucs, axis=1)
        return np.mean(perm_aucs, axis=1), perm_aucs
    ...
    perm_results = Parallel(n_jobs=N_JOBS)(delayed(one_perm)(pi) for pi in range(N_PERMS))
    perm_dist_mean = np.stack([item[0] for item in perm_results], axis=0)
