...
TASKS = [
    ...
    # },
    {
        ...
        'folder': 'task1_color_vs_gray_per_category3'
    },
    # {
...
def main():
    ...
    batch_root = str(get_within_decoding_batch_dir(BASE_PATH, 'highgamma', SUBJECT, batch_name=BATCH_NAME))
    ...
    task_outputs = {}
    for task in TASKS:
        output_dir = str(get_within_decoding_task_dir(BASE_PATH, task['id'], 'highgamma', SUBJECT, get_perm_tag(), variant='with_sti', batch_name=BATCH_NAME))
...
def build_task1_pair_holdout_data(mat):
    if 'hg_task1' not in mat:
        raise ValueError('Missing matrix: hg_task1')
    data = mat['hg_task1']
    if data.shape[0] < 8:
...
def build_task2_gray_memory_combos(mat):
    if 'hg_task2' not in mat:
        raise ValueError('Missing matrix: hg_task2')
    data = mat['hg_task2']
    gray_green = [2, 5]
    gray_red = [8, 11]
