function Sec2_5_preprocess_gamma()
% PREPROCESS_HIGgAMMA 提取每个ROI的High-Gamma (70-150Hz) 信号功率包络。
%
% 步骤:
%   1. 识别Task1/2/3之间的公共通道
%   2. 按ROI分组通道
%   3. 对原始信号进行70-150Hz带通滤波 → Hilbert变换取能量包络
%   4. 对能量包络进行时间平滑（高斯窗）
%   5. 裁剪至 -100~1000ms 并按ROI保存
%
% 输出格式:
%   每个ROI保存为一个 .mat 文件，包含:
%     - g_task1: [Cond, Rep, Ch, Time] 的High-Gamma能量
...
save_dir = fullfile(paths.feature_root, 'gamma', subject);
if ~exist(save_dir, 'dir'), mkdir(save_dir); end

loc_file = fullfile(data_dir, sprintf('%s_ieegloc.xlsx', subject));

% 日志文件
fid = fopen(fullfile(save_dir, 'preprocess_higgamma_log.txt'), 'w');
if fid == -1, error('无法创建日志文件'); end

fprintf(fid, '=== High-Gamma预处理开始 ===\n');
fprintf(fid, '被试: %s\n', subject);
fprintf(fid, '频段: 70-150Hz\n\n');
...
try
    ...
    for t = 1:length(tasks)
        ...
        for r = 1:length(rois)
            ...
            g_data = permute(power_reshaped, [2, 3, 4, 1]);
            
            % --- 保存 ---
            safe_roi = matlab.lang.makeValidName(roi_name);
            roi_file = fullfile(save_dir, sprintf('%s.mat', safe_roi));
            
            var_name = sprintf('g_%s', task_name);
            eval(sprintf('%s = g_data;', var_name));
