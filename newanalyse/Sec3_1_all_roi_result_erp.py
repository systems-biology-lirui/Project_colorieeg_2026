...
def main():
    ...
    batch_root = str(get_within_decoding_batch_dir(BASE_PATH, 'erp', SUBJECT, batch_name=BATCH_NAME))
    ...
    task_outputs = {}
    for task in TASKS:
        output_dir = str(get_within_decoding_task_dir(BASE_PATH, task['id'], 'erp', SUBJECT, get_perm_tag(), variant='with_sti', batch_name=BATCH_NAME))
...
def build_task2_gray_memory_combos(mat):
    if 'erp_task2' not in mat:
        raise ValueError('Missing matrix: erp_task2')
    data = mat['erp_task2']
    gray_green = [2, 5]
    gray_red = [8, 11]
