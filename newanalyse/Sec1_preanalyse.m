...
ENABLE_BAD_EPOCH_REJECTION = false;
% 启动并重置 EEGLAB 环境
[ALLEEG EEG CURRENTSET ALLCOM] = eeglab;
...
if ENABLE_BAD_EPOCH_REJECTION
    ...
    fprintf('--- 跳过 SEEG 自动坏段剔除 (ENABLE_BAD_EPOCH_REJECTION = false) ---\n');
    bad_epochs = [];
    bad_epoch_stats = struct();
end
bad_epoch_indices_saved = bad_epochs(:)';
total_epoch_count_before_bad = EEG_QC.trials;
all_epoch_eventtypes_before_bad = {EEG_QC.epoch.eventtype};
...
erp_selection_meta = build_epoch_selection_metadata(all_epoch_eventtypes_before_bad, bad_epoch_indices_saved, target_event, min_trials);
epoch.condition_original_count = erp_selection_meta.condition_original_count;
epoch.condition_kept_count_before_trim = erp_selection_meta.condition_kept_count_before_trim;
epoch.condition_original_repeat_index = erp_selection_meta.condition_original_repeat_index;
epoch.condition_global_epoch_index = erp_selection_meta.condition_global_epoch_index;
epoch.min_trials = min_trials;
...
tfa_selection_meta = build_epoch_selection_metadata(all_epoch_eventtypes_before_bad, bad_epoch_indices_saved, target_event, min_trials_tfa);
epoch.condition_original_count = tfa_selection_meta.condition_original_count;
epoch.condition_kept_count_before_trim = tfa_selection_meta.condition_kept_count_before_trim;
epoch.condition_original_repeat_index = tfa_selection_meta.condition_original_repeat_index;
epoch.condition_global_epoch_index = tfa_selection_meta.condition_global_epoch_index;
epoch.min_trials = min_trials_tfa;
...
n_cond = length(target_event);
meta = struct(...
    'condition_original_count', zeros(1, n_cond), ...
    'condition_kept_count_before_trim', zeros(1, n_cond), ...
    'condition_original_repeat_index', zeros(n_cond, n_keep), ...
    'condition_global_epoch_index', zeros(n_cond, n_keep));

for i = 1:n_cond
    condition_global_indices = find(strcmp(target_event{i}, all_epoch_eventtypes));
    kept_mask = ~ismember(condition_global_indices, removed_bad_epochs);
    kept_global_indices = condition_global_indices(kept_mask);
    kept_repeat_indices = find(kept_mask);

    meta.condition_original_count(i) = length(condition_global_indices);
    meta.condition_kept_count_before_trim(i) = length(kept_repeat_indices);

    if length(kept_repeat_indices) < n_keep
        error('条件 %s 在坏段剔除后仅剩 %d 个 trial，无法填充 %d 个保留索引。', ...
            target_event{i}, length(kept_repeat_indices), n_keep);
    end

    meta.condition_original_repeat_index(i, :) = kept_repeat_indices(1:n_keep);
    meta.condition_global_epoch_index(i, :) = kept_global_indices(1:n_keep);
end
end
